...
def test_logging(logger: tb.SummaryWriter):
    ...
    for epoch in range(10):
        ...
        avg_train_accuracy = torch.mean(torch.cat(metrics["train_acc"]))
        logger.add_scalar("train_accuracy", avg_train_accuracy.item(), global_step)
        ...
        avg_val_accuracy = torch.mean(torch.cat(metrics["val_acc"]))
        logger.add_scalar("val_accuracy", avg_val_accuracy.item(), global_step)
